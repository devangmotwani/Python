...
loc = 1
while True:
    availableExits = ", ".join(locations[loc]["exits"].keys()) #Solved

    print(locations[loc]["desc"]) #Solved

    if loc == 0:
        break
    else:
        allExits = locations[loc]["exits"].copy()       #Solved
        allExits.update(locations[loc]["namedExits"])   #Solved
